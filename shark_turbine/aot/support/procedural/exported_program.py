...
class _Hooks(FxImporterHooks):
    ...
    def resolve_literal(
        self, gni: GraphNodeImporter, literal: Any, info: InputInfo | None = None
    ) -> Optional[Value]:
    ...
    def _lift_tensor_to_global(
        ...
        if (
            not self._should_lift_tensor_to_global(literal, external_trait, info)
        ):
            return None
    ...
    def _should_lift_tensor_to_global(
        self, literal: torch.Tensor, external_trait: Optional[ExternalTensorTrait], info: InputInfo | None 
    ) -> bool:
